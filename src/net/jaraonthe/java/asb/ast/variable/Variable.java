...
public class Variable extends VariableLike
{
    ...
    public enum Type
    {
         ...
         */
        LABEL('\u0011', "/label", false, false), // Same signatureMarker as REGISTER, as in the
                                                 // resolvingSignature they have to be the same
                                                 // (because syntactically they cannot be
                                                 // distinguished in the invocation without going
                                                 // through the resolving process). In signature
                                                 // they can be distinguished because one has length
                                                 // but not the other.
        /**
        ...
        /**
         * True if this type of parameter has length information.
         */
        ...
        if (type.hasLength) {
            if (!Constraints.isValidLength(length)) {
                throw new IllegalArgumentException(
                    "Invalid parameter " + name + " length. Given value is: " + length
                );
            }
        } else if (length != -1) {
            throw new IllegalArgumentException(
